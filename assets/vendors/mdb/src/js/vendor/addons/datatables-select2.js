...
DataTable.select.init = function ( dt ) {
	...
	$.fn.dataTable.ext.order['select-checkbox'] = function ( settings, col ) {
		return this.api().column( col, {order: 'index.html.twig'} ).nodes().map( function (td ) {
...
function enableMouseSelection ( dt )
{
	...
	container
		.on('click.dtSelect', 'th.select-checkbox-all', function(e) {
			e.preventDefault();
			e.stopPropagation();
            e.stopImmediatePropagation();
			
			$(this).parent().toggleClass('selected').closest('table').find('td.select-checkbox').parent().toggleClass('selected');

			var cell = dt.context[0].aoData.length;

			for(let i=0; i<=cell; i++) {
				typeSelect ( e, dt, ctx, 'row', i )
			}			
		})
