class PartenaireList {
    constructor(container) {
        this.container = $(container);
    }

    #api = new Api(($('body').data('apiDomainName') + '/api/partenaires/'));
        ...
        try {
            ...
            if (parseInt(data['hydra:totalItems']) === 0)
                ...
                data['hydra:member'].map((article) => {
                        this.container.append((new Partenaire(article)).showTag());
                });
            this.addEvent();
        } catch (e) {
            console.log(e);
    ...
    addEvent() {
        ...
        $('.js-edit').on('click',{api: this.#api}, (e) => {
            $.cookie('edit_partenaire_id', $(e.currentTarget).data('id'));
            window.location.pathname = '/backend/editer-un-partenaire/';
        });
