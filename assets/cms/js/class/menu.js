class Menu {
    ...
    showTag() {
        let checkbox = '<input class="form-check-input mt-1 ml-1 js-checkbox" data-slug="' + this.slug + '" data-id="' + this.id + '" type="checkbox"/>',
            ...
            btnaddSubMenu = '<button class="btn btn-outline-dark mt-0 mb-0 js-add-sub-menu" data-slug="' + this.slug + '" data-id="' + this.id + '" data-title="' + this.title + '" data-@id="' + this['@id'] + '"><i class="fa fa-indent" aria-hidden="true"></i></button>'
        ;
        return '<tr id="' + this.slug + '">' + tdCheckbox + tdTitle + '<td>01/01/2020</td>' + tdSlug + '<td><div class="btn-group btn-group-sm" role="group">' + btnDelete + btnEnable + btnEdit + btnaddSubMenu + '</div> </td></tr>';
    }
}
