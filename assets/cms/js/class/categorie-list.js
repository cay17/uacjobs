class CategorieList {
    ...
    #api = new Api(($('body').data('apiDomainName') + '/api/faq_categories/'));
        ...
        try {
            ...
            if (parseInt(data['hydra:totalItems']) === 0)
                ...
                data['hydra:member'].map((article) => {
                        this.container.append((new Categorie(article)).showTag());
                });
            this.addEvent();
        } catch (e) {
            console.log(e);
    ...
    addEvent() {
        ...
        $('.js-edit').on('click',{api: this.#api}, (e) => {
            $.cookie('edit_categorie_id', $(e.currentTarget).data('id'));
            window.location.pathname = '/backend/editer-une-categorie/';
        });
