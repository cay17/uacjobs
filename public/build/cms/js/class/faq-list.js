class FaqList {
    ...
    #api = new Api(($('body').data('apiDomainName') + '/api/faqs/'));
        ...
        try {
            ...
            if (parseInt(data['hydra:totalItems']) === 0)
                ...
                data['hydra:member'].map((article) => {
                        this.container.append((new Faq(article)).showTag());
                });
            this.addEvent();
        } catch (e) {
            console.log(e);
    ...
    addEvent() {
        ...
        $('.js-edit').on('click',{api: this.#api}, (e) => {
            $.cookie('edit_faq_id', $(e.currentTarget).data('id'));
            window.location.pathname = '/backend/editer-une-question/';
        });
